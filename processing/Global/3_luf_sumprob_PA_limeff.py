...
ap.add_argument('-s', '--scenario', type=str,
                required=True, help="e.g., rcp26 or rcp60")
ap.add_argument('-y', '--year', type=str, required=True, help="e.g., 2080")
ap.add_argument('--type', type=str,
                choices=['baseline', 'pa30_bio', 'pa30_bcw', 'climate_only'],
                required=True)
args = ap.parse_args()
...
ssp = ssp_map.get(args.scenario)
if not ssp:
    print(f" ERROR: Scenario {args.scenario} not mapped to an SSP.")
    sys.exit(1)

# Logic for Future Scenarios
if args.type == 'baseline':
    ...
    # Note: Climate only uses the 2015 land use mask for the 2080 richness
    LU_FILE = os.path.join(DATA_DIR, "lim_eff_control_wdpa17_2015_hist.nc")
    SUFFIX = "clim_only"
...
for sdm in args.model:
    for gcm in args.gcm:
        for taxa in args.taxa:
            ...
            base_name = f"lim_eff_summed_prob_{taxa}_{sdm}_{gcm}_{args.scenario}_{args.year}_{SUFFIX}"
            ...
            if not os.listdir(dir_sp):
                print(f"Directory empty or not found: {dir_sp}")
                continue
            ...
            print(f"FINISHED: {base_name}.nc | Total: {processed_count}")
