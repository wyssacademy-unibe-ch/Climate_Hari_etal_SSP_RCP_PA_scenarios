...
# Run with:
# nohup bash \
# /capacity/occr_davin/mguzman/chari_P2_review/scripts_git/processing/Endemics/bash/run_luf_30_funding_constrained.sh \
# > /capacity/occr_davin/mguzman/chari_P2_review/scripts/logs/nohup_luf_pa30_funding.log \
# 2>&1 &
...
LOG_FILE="${LOG_DIR}/run_luf_endemics_funding_${RUN_DATE}.log"
PYTHON_EXECUTABLE="python3"

# false: skip existing outputs only when both NetCDF variables are valid.
...
if missing:
    raise SystemExit(f"ERROR: Match file is missing columns: {sorted(missing)}")
available = set(data["taxon"].dropna().astype(str).str.strip())
...
get_expected_output_path() {
    ...
    case "$pa_type" in
        ...
        "pa30_bcw")
            ...
            ;;
        *)
            printf "ERROR: Unsupported PA type: %s\n" "$pa_type" >&2
            return 1
            ;;
    esac
...
try:
    with xr.open_dataset(path, decode_times=False) as dataset:
        ...
        if subset != "endemic_species":
            raise ValueError(f"unexpected species_subset attribute: {subset!r}")
...
run_luf_job() {
    ...
    # This is the change that enables 12 simultaneous processes.
    # As soon as 12 jobs have been launched, wait for that batch,
    # validate every output, then launch the next batch.
...
log_msg "======================================================="
log_msg "STARTING ENDEMIC FUNDING-CONSTRAINED LUF WORKFLOW"
log_msg "Start: $(date)"
...
# Wait for the final partial batch, which may contain fewer than 12 jobs.
if ! wait_for_current_batch; then
...
log_msg "ALL REQUESTED ENDEMIC FUNDING-CONSTRAINED OUTPUTS ARE PRESENT AND VALID."
